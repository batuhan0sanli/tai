...
- `config init` / `config path`: write a starter config (all providers stubbed) / print the config file path. See **Providers & config** below.
...
- **`cmd/config_test.go`** — `runConfigInit` / `runConfigPath` through the `configFilePath` / `configSave` injection points (`withConfigInjections`): new-file write, refuse-existing, `--force` overwrite, filepath error, save error, path print + error.
